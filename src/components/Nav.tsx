...
export default function Nav() {
  ...
  const active = (path: string) =>
    path == location.pathname
      ? "text-sm text-white hover:text-neutral-300 md:py-4 focus:outline-none focus:text-neutral-300 underline"
        ...
        >
          <div class="flex flex-col gap-y-4 gap-x-0 mt-5 md:flex-row md:items-center md:justify-end md:gap-y-0 md:gap-x-7 md:mt-0 md:ps-7">
            <For
              each={[
                { name: "Home", path: "/" },
                { name: "Letter to Congress", path: "/letter-to-congress" },
                { name: "Call to Congress", path: "/call-to-congress" },
